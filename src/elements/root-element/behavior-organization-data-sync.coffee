...
app.behaviors.local['root-element'].organizationDataSync =
  ...
  _syncOrganizationData: (params, cbfn)->
    ...
    organizationId = (app.db.find 'organization')[0].idOnServer
    return console.error 'OrganizationId is required for sync' unless organizationId
          ...
          if response.hasError
            ...
            for item in serverToClientItemList
              unless item.lastModifiedDatetimeStamp
                continue
              app.db.upsert collectionName, item, ({serial, lastModifiedDatetimeStamp})-> item.serial is serial
