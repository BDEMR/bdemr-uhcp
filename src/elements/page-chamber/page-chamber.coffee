...
Polymer {
  ...
  _generateTimeSlotList: (startTimeString, endTimeString, bookingSlotSizeInMinutes)->
    ...
    while (start.getTime() < end.getTime())
      ...
      start.setMinutes(start.getMinutes() + bookingSlotSizeInMinutes)
  ...
  managePatientsScheduleTapped: (e)->
    { entry } = e.model
    dateString = this._getMonthString() + '-' + (if entry.date < 10 then '0' else '') + entry.date
    this.domHost.navigateToPage "#/chamber-patients/chamber:#{this.chamberName}/date:#{dateString}"
  ...
  navigatedIn: ->
    ...
    this.chamberName = this.domHost.getPageParams()['which']
