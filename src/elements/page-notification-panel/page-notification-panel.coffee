Polymer {
  ...
  _searchUser: ->
    # Patient search
    if @roleSelected is 0
      @_patientSearch (data)=>
        if data.length > 0
          userSuggestionArray = ({text:"#{item.name}-#{item.email}-#{item.phone}", value:item.idOnServer} for item in data)
          @.$.userSearch.suggestions userSuggestionArray
        else
          @domHost.showToast 'No Match Found'
    else if @roleSelected is 1
      @_doctorSearch (data)=>
        if data.length > 0
          userSuggestionArray = ({text:"#{item.name}-#{item.email}-#{item.phone}", value:item.idOnServer} for item in data)
          @.$.userSearch.suggestions userSuggestionArray
      ...
      @domHost.showModalDialog 'Work In Progress'
      # @_clinicSearch (data)=>
      #   if data.length > 0
      #     userSuggestionArray = ({text:"#{item.name}-#{item.email}-#{item.phone}", value:item.idOnServer} for item in data)
      #     @.$.userSearch.suggestions userSuggestionArray
