...
Polymer {
  ...
  _setChamberList: (chamberList, cbfn)->
    data = { 
      apiKey: this.user.apiKey
    }
    Object.assign(data, {chamberList})
    this.callApi '/bdemr-booking--doctor--set-chamber-list', data, (err, response)=>
      if response.hasError
        this.domHost.showModalDialog response.error.message
      else
        cbfn()
  ...
  chamberSaveTapped: (e)->
    ...
    if chamber.startTimeString >= chamber.endTimeString
      ...
      return
    this._checkIfUnique chamber.shortCode, chamber.name, =>
      chamber._isEditMode = false
      # Figuring Out if we should update or insert
      chamberIndexList = (index for item, index in @chamberList when (item.shortCode is chamber.shortCode))
      if chamberIndexList.length then @chamberList.splice chamberIndexList[0], 1, chamber else @chamberList.push chamber
      this._setChamberList this.chamberList, =>
  ...
  chamberManageTapped: (e)->
    { chamber } = e.model
    this.domHost.navigateToPage "#/chamber/which:#{chamber.name}"
